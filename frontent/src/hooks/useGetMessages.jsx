...
//   const { nextCursor, hasMore, loading } = useSelector(store => store.message); // Added loading here!
...
//       try {
//         const res = await axios.get(`http://localhost:8001/test/getMessage/${selectedUser._id}`, { withCredentials: true });
//         if (res.data.success) {
...
//           }));
//           axios.post(`http://localhost:8001/test/mark-read/${selectedUser._id}`, {}, { withCredentials: true }).catch(()=>{});
//         }
//       } catch (err) {
...
//     // ■ GUARD: If already loading, stop immediately! Prevents infinite loops.
//     if (!hasMore || !nextCursor || !selectedUser?._id || loading) return;
    
//     dispatch(setLoading(true));
//     try {
//       const res = await axios.get(`http://localhost:8001/test/getMessage/${selectedUser._id}?cursor=${nextCursor}`, { withCredentials: true });
//       if (res.data.success) {
//         const reversed = [...res.data.messages].reverse();
